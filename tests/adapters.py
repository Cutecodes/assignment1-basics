...
def run_linear(
    ...
    ln.weight = nn.Parameter(weights)
...
def run_multihead_self_attention(
    ...
    """
    attention = Attention(d_model, num_heads, num_heads)
    ...
    attention.o_proj.weight.data = o_proj_weight
...
def run_multihead_self_attention_with_rope(
    ...
    attention =  Attention(d_model, num_heads, num_heads, rope)
    attention.q_proj.weight.data = q_proj_weight
    attention.k_proj.weight.data = k_proj_weight
    attention.v_proj.weight.data = v_proj_weight
    attention.o_proj.weight.data = o_proj_weight
...
def run_transformer_lm(
    ...
    for i in range(num_layers):
        transformer_lm.transformer_layers[i].ln1.weight.data = weights[f"layers.{i}.ln1.weight"]
        transformer_lm.transformer_layers[i].attn.q_proj.weight.data = weights[f"layers.{i}.attn.q_proj.weight"]
        transformer_lm.transformer_layers[i].attn.k_proj.weight.data = weights[f"layers.{i}.attn.k_proj.weight"]
        transformer_lm.transformer_layers[i].attn.v_proj.weight.data = weights[f"layers.{i}.attn.v_proj.weight"]
        transformer_lm.transformer_layers[i].attn.o_proj.weight.data = weights[f"layers.{i}.attn.output_proj.weight"]

        transformer_lm.transformer_layers[i].ln2.weight.data = weights[f"layers.{i}.ln2.weight"]
        transformer_lm.transformer_layers[i].ffn.w1.weight.data = weights[f"layers.{i}.ffn.w1.weight"]
        transformer_lm.transformer_layers[i].ffn.w2.weight.data = weights[f"layers.{i}.ffn.w2.weight"]
        transformer_lm.transformer_layers[i].ffn.w3.weight.data = weights[f"layers.{i}.ffn.w3.weight"]
    
    transformer_lm.ln_final.weight.data = weights[f"ln_final.weight"]
    transformer_lm.lm_head.weight.data = weights[f"lm_head.weight"]
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    bpe = BPETokenizerTrainer(input_path, vocab_size, special_tokens)
    bpe.train()
